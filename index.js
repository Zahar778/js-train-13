...
function manipulateUrl(url) {
  ...
  return {href: urlObject.href, protocol: urlObject.protocol, host: urlObject.host, pathname: urlObject.pathname, search: urlObject.search, params: params};
...
function manipulateQuery(url, options) {
    ...
    // Перевіряємо, чи є налаштування для додавання параметрів (append)
    if (options.append) {
      for (const [key, value] of options.append.entries()) {
        urlObj.searchParams.append(key, value);
      }
    }
  
    // Перевіряємо, чи є налаштування для видалення параметрів (delete)
    if (options.delete()) {
...
function sortUrlParams(url) {
  ...
  urlObj.search = new URLSearchParams(paramsArray).toString();
